import {
  ...
  function TopPerformers({ match }) {
    const colorScheme = useColorScheme();
  
    const topPerformers = ["Big Bob"]; // Top performers for the winning department
  ...
  export default function HistroyScreen() {
    ...
    return (
      <ThemedView className="flex flex-col items-center ">
        <ThemedText type="title" className="mt-8 px-1 justify-center items-center text-center">
        ...
        <ThemedView className="px-5 flex flex-col mt-6 w-full">
          {matches
            .filter(
              (item) =>
                item.match === games[activeGameState].name &&
                item.year === selectedYear &&
                item.winner // Only winners
            )
            .map((item, index) => (
              <TopPerformers match={item} key={index} />
            ))}
        </ThemedView>
      </ThemedView>
    );
